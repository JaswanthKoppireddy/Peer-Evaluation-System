 ...
 */
export const checkAnomaly = async (submissionId: string): Promise<boolean> => {
  try {
    const evaluations = await Evaluation.find({ submissionId });
    if (evaluations.length < 3) return false; // Need sufficient data points
    ...
    if (stdDev === 0) return false; // All scores are identical
